		...
		if (hexLoaded)
		{
			...
			if (ret || !argError)
			{
				if(immVerify && immVerified) {
					__tprintf("Immediate Verify Succeeded.\n");
				}
				else if (verify)
				{
					if (noProgEntryForVerify)
	...
	if (checkSwitch(argv[i]))
	{
		switch (argv[i][1])
		{
			...
			case 'M':
				...
				printf("immediately followed by memory region parameter(s). Valid parameters are:\n");
				...
				printf("without being first erased. \"Program Succeeded\" will be displayed if the\n");
				printf("operation is successful, otherwise the first address of the first memory\n");
				printf("region where programming failed will be displayed along with a description of\n");
				printf("the failure. When using region parameters, an additional '+' parameter may be\n");
				printf("used. This enables immediate verify, which is useful if the loaded hex file\n");
				printf("has code and/or data protection enabled in configuration memory.\n");
				printf("NOTE: For HCS and serial EEPROM devices, memory is considered region 'P'\n");
				printf("\n");
				printf("This command may be used without parameters or with memory region(s).\n");
				printf("\n");
			...
			case 'W':
				...
				printf("to 5.0 Volts, but with some luck it can work down to 1.8 Volts.\n");
				printf("PICkit 3 works with an external power source voltage range of 1.7 Volts to\n");
				printf("5.0 Volts.\n");
				printf("\n");
				printf("There is an optional parameter to specify threshold for the external power\n");
				printf("source voltage. If specified, the programming operations will not proceed\n");
				printf("until the external power source voltage has risen above the threshold.\n");
				printf("\n");
				printf("Syntax Examples -w\n");
				printf("                -w4.5\n");
				break;
...
void Ccmd_app::displayHelp(void)
{
	...
	printf("        boborjan2 for libusb-1.0 support on linux and other improvements\n");
